 ...
 */
package com.adpf.seats.restful;
 ...
 */
@RestController
@RequestMapping("/api/adpf/seats")
public class AdvidSeatsidRestful {

	protected static final Logger logger = LogManager.getLogger(AdvidSeatsidRestful.class);
    
    @Autowired
    private AdvidSeatsidService advidSeatsidService;
	...
	public JSONObject getList(HttpServletRequest request, @RequestParam Map<String, Object> params){
		...
		try{
			...
			if(userInfo == null) json = RestfulRetUtils.getErrorNoUser();
				...
				json = advidSeatsidService.getList(pageable, advidSeatsid, params);
			}
		}catch(Exception e){
	...
	public JSONObject add(HttpServletRequest request, @RequestBody Map<String, Object> params){
		...
		try{
			...
			if(userInfo == null) json = RestfulRetUtils.getErrorNoUser();
				...
				json = advidSeatsidService.add(params, userInfo);
			}
		}catch(Exception e){
	...
	public JSONObject edit(HttpServletRequest request, @RequestBody Map<String, Object> params){
		...
		try{
			...
			if(userInfo == null) json = RestfulRetUtils.getErrorNoUser();
				...
				json = advidSeatsidService.edit(params, userInfo);
			}
		}catch(Exception e){
	...
	public JSONObject del(HttpServletRequest request,@RequestBody String idsList){
		...
		try {
			...
			if(userInfo == null) json = RestfulRetUtils.getErrorNoUser();
				...
				json = advidSeatsidService.del(idsList, userInfo);
			}
		} catch (Exception e) {
